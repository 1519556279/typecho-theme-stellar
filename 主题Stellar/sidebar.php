        ...
        <?php if ($hasSocial): ?>
            <div class="widget-social">
                <?php foreach ($social as $key => $cfg): ?>
                    ...
                    <?php if (!empty($val)): ?>
                        <?php $type = $cfg[0]; $copy = ''; ?>
                        <?php if ($type === 'mail'): $href = 'mailto:' . $val; ?>
                        <?php elseif ($type === 'wechat'): $href = '#'; $copy = $val; ?>
                        <?php elseif ($type === 'qq'): $href = 'https://wpa.qq.com/msgrd?v=3&uin=' . $val . '&site=qq&menu=yes'; ?>
                        <?php elseif ($type === 'telegram'): $href = 'https://t.me/' . ltrim($val, '@'); ?>
                        <?php else: $href = $val; ?>
                        <?php endif; ?>
                        <a href="<?php echo $href; ?>"<?php echo $copy ? ' data-copy="' . htmlspecialchars($copy) . '"' : ''; ?><?php echo $type === 'wechat' ? '' : ' target="_blank" rel="noopener"'; ?> title="<?php echo $cfg[1]; ?>">
                            ...
                            <?php elseif ($cfg[0] === 'bili'): ?><svg viewBox="0 0 24 24" width="16" height="16" fill="currentColor"><path d="M17.8 3.7l1.8-1.8c.3-.3.3-.8 0-1.1-.3-.3-.8-.3-1.1 0L16.6 2.6c-.8-.4-1.7-.6-2.6-.6H10c-.9 0-1.8.2-2.6.6L5.5.8c-.3-.3-.8-.3-1.1 0-.3.3-.3.8 0 1.1l1.8 1.8C3.4 5.4 1.5 8 1.5 11v7.5c0 1.7 1.3 3 3 3h15c1.7 0 3-1.3 3-3V11c0-3-1.9-5.6-4.7-7.3zM7.5 8.2h9c.4 0 .8.4.8.8s-.4.8-.8.8h-9c-.4 0-.8-.4-.8-.8s.4-.8.8-.8zm1.8 8.8c-.8 0-1.5-.7-1.5-1.5s.7-1.5 1.5-1.5 1.5.7 1.5 1.5-1.5-.7 1.5-1.5 1.5zm5.4 0c-.8 0-1.5-.7-1.5-1.5s.7-1.5 1.5-1.5 1.5.7 1.5 1.5-1.5-.7 1.5-1.5 1.5z"/></svg>
